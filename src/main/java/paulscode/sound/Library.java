...
public class Library {
     ...
     */
    protected HashMap<String, paulscode.sound.SoundBuffer> bufferMap = null;
    /**
     * Map containing all created sources for easy look-up by name.
     */
    protected HashMap<String, paulscode.sound.Source> sourceMap;  // (name, source data) pairs
    /**
     ...
     */
    protected paulscode.sound.StreamThread streamThread;
    /**
    ...
    public Library() throws SoundSystemException {
        ...
        bufferMap = new HashMap<String, paulscode.sound.SoundBuffer>();

        // instantiate the source map:
        sourceMap = new HashMap<String, paulscode.sound.Source>();
    ...
    public void rawDataStream(AudioFormat audioFormat, boolean priority,
        ...
        sourceMap.put(sourcename,
                new paulscode.sound.Source(audioFormat, priority, sourcename, posX,
    ...
    public void newSource(boolean priority, boolean toStream, boolean toLoop,
                          String sourcename, paulscode.sound.FilenameURL filenameURL,
                          float posX, float posY, float posZ, int attModel,
                          float distOrRoll) {
        sourceMap.put(sourcename,
                new paulscode.sound.Source(priority, toStream, toLoop, sourcename,
    ...
    public void setTemporary(String sourcename, boolean temporary) {
        paulscode.sound.Source mySource = sourceMap.get(sourcename);
        if (mySource != null)
    ...
    public float millisecondsPlayed(String sourcename) {
        ...
        if (midiSourcename(sourcename)) {
            ...
            paulscode.sound.Source source = sourceMap.get(sourcename);
            if (source == null) {
     ...
     * @return Number of prior buffers that have been processed, or -1 if unable to queue the buffer (if the source was culled, for example).
     */
    public int feedRawAudioData(paulscode.sound.Source source, byte[] buffer) {
        if (source == null) {
     ...
     */
    public void play(paulscode.sound.Source source) {
        if (source == null)
            return;
     ...
     */
    public void copySources(HashMap<String, paulscode.sound.Source> srcMap) {
        if (srcMap == null)
            return;
        Set<String> keys = srcMap.keySet();
        Iterator<String> iter = keys.iterator();
        String sourcename;
        paulscode.sound.Source srcData;
        ...
        while (iter.hasNext()) {
            ...
            if (srcData != null) {
                ...
                sourceMap.put(sourcename, new paulscode.sound.Source(srcData, null));
            }
        }
    }
     ...
     */
    private Channel getNextChannel(paulscode.sound.Source source) {
        if (source == null)
            return null;
     ...
     */
    public void queueSound(String sourcename, paulscode.sound.FilenameURL filenameURL) {
        if (midiSourcename(sourcename)) {
     ...
     */
    public void fadeOut(String sourcename, paulscode.sound.FilenameURL filenameURL,
                        long milis) {
        if (midiSourcename(sourcename)) {
     ...
     */
    public void fadeOutIn(String sourcename, paulscode.sound.FilenameURL filenameURL,
     ...
     */
    public paulscode.sound.Source getSource(String sourcename) {

...
public class SoundSystem {
     ...
     */
    private static paulscode.sound.SoundSystemException lastException = null;
    /**
     ...
     */
    protected paulscode.sound.SoundSystemLogger logger;
    /**
     * Handle to the active sound library.
     */
    protected paulscode.sound.Library soundLibrary;
    /**
     * List of queued commands to perform.
     */
    protected List<paulscode.sound.CommandObject> commandQueue;
    /**
     * Processes queued commands in the background.
     */
    protected paulscode.sound.CommandThread commandThread;
    /**
     * Name of this class.
     */
     ...
     * methods.
     */
    private List<paulscode.sound.CommandObject> sourcePlayList;
    ...
    public SoundSystem() {
        ...
        if (logger == null) {
            logger = new paulscode.sound.SoundSystemLogger();
        ...
        if (libraries != null) {
            ...
            while (i.hasNext()) {
                ...
                try {
                    init(c);
                    return;
                } catch (paulscode.sound.SoundSystemException sse) {
                    logger.printExceptionMessage(sse, 1);
                }
            }
        }
        try {
            init(paulscode.sound.Library.class);
            return;
        } catch (paulscode.sound.SoundSystemException sse) {
            logger.printExceptionMessage(sse, 1);
        }
     ...
     */
    public SoundSystem(Class libraryClass) throws paulscode.sound.SoundSystemException {
        // create the message logger:
        logger = SoundSystemConfig.getLogger();
        // if the user didn't create one, then do it now:
        if (logger == null) {
    ...
    public static boolean libraryCompatible(Class libraryClass) {
        // create the message logger:
        paulscode.sound.SoundSystemLogger logger = SoundSystemConfig.getLogger();
        // if the user didn't create one, then do it now:
        if (logger == null) {
     ...
     */
    public static paulscode.sound.SoundSystemException getLastException() {
     ...
     */
    public static void setException(paulscode.sound.SoundSystemException e) {
        lastException(SET, e);
    }
     ...
     */
    private static paulscode.sound.SoundSystemException lastException(boolean action,
                                                                      paulscode.sound.SoundSystemException e) {
        synchronized (SoundSystemConfig.THREAD_SYNC) {
            if (action == SET)
                lastException = e;
     ...
     *                     information about chosing a sound library.
     */
    protected void init(Class libraryClass) throws paulscode.sound.SoundSystemException {
        message("", 0);
        ...
        commandQueue = new LinkedList<paulscode.sound.CommandObject>();
        // create the working source playlist:
        sourcePlayList = new LinkedList<paulscode.sound.CommandObject>();
    ...
    public void loadSound(String filename) {
        ...
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.LOAD_SOUND,
                new paulscode.sound.FilenameURL(filename)));
        // Wake the command thread to process commands:
        commandThread.interrupt();
    }
    ...
    public void loadSound(URL url, String identifier) {
        ...
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.LOAD_SOUND,
                new paulscode.sound.FilenameURL(url, identifier)));
        // Wake the command thread to process commands:
        commandThread.interrupt();
    }
    ...
    public void loadSound(byte[] data, AudioFormat format, String identifier) {
        // Queue a command to load the sound file from a URL:
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.LOAD_DATA,
                identifier,
                new paulscode.sound.SoundBuffer(data, format)));
        // Wake the command thread to process commands:
        commandThread.interrupt();
    }
    ...
    public void unloadSound(String filename) {
        ...
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.UNLOAD_SOUND, filename));
        // Wake the command thread to process commands:
        commandThread.interrupt();
    }
    ...
    public void queueSound(String sourcename, String filename) {
        ...
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.QUEUE_SOUND, sourcename,
                new paulscode.sound.FilenameURL(filename)));
        // Wake the command thread to process commands:
        commandThread.interrupt();
    }
    ...
    public void dequeueSound(String sourcename, String filename) {
        ...
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.DEQUEUE_SOUND,
                sourcename, filename));
        // Wake the command thread to process commands:
        commandThread.interrupt();
    }
    ...
    public void fadeOut(String sourcename, String filename, long milis) {
        paulscode.sound.FilenameURL fu = null;
        if (filename != null)
            fu = new paulscode.sound.FilenameURL(filename);
        // Queue a command to fade out:
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.FADE_OUT, sourcename, fu,
                milis));
        // Wake the command thread to process commands:
        commandThread.interrupt();
    }
    ...
    public void fadeOut(String sourcename, URL url, String identifier,
        ...
        if (url != null && identifier != null)
            fu = new paulscode.sound.FilenameURL(url, identifier);
        // Queue a command to fade out:
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.FADE_OUT, sourcename, fu,
                milis));
        // Wake the command thread to process commands:
    ...
    public void fadeOutIn(String sourcename, String filename, long milisOut,
                          long milisIn) {
        // Queue a command to load the sound file:
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.FADE_OUT_IN,
                sourcename,
                new paulscode.sound.FilenameURL(filename), milisOut,
                milisIn));
        // Wake the command thread to process commands:
        commandThread.interrupt();
    }
    ...
    public void checkFadeVolumes() {
        ...
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.CHECK_FADE_VOLUMES));
        // Wake the command thread to process commands:
        commandThread.interrupt();
    }
    ...
    public void backgroundMusic(String sourcename, String filename,
        ...
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.QUICK_PLAY, true,
                true, toLoop, sourcename,
                new paulscode.sound.FilenameURL(filename), 0, 0, 0,
                SoundSystemConfig.ATTENUATION_NONE, 0, false));
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.PLAY, sourcename));
    ...
    public void newSource(boolean priority, String sourcename, String filename,
        ...
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.NEW_SOURCE, priority,
                false, toLoop, sourcename,
                new paulscode.sound.FilenameURL(filename), x, y, z,
                attmodel, distOrRoll));
        commandThread.interrupt();
    }
    ...
    public void rawDataStream(AudioFormat audioFormat, boolean priority,
                              ...
                              int attModel, float distOrRoll) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.RAW_DATA_STREAM,
    ...
    public String quickPlay(boolean priority, String filename, boolean toLoop,
        ...
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.QUICK_PLAY, priority,
                false, toLoop, sourcename,
                new paulscode.sound.FilenameURL(filename), x, y, z,
    ...
    public void setPosition(String sourcename, float x, float y, float z) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_POSITION,
                sourcename, x, y, z));
        commandThread.interrupt();
    }
    ...
    public void setVolume(String sourcename, float value) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_VOLUME,
                sourcename, value));
        commandThread.interrupt();
    }
    ...
    public void setPitch(String sourcename, float value) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_PITCH,
                sourcename, value));
        commandThread.interrupt();
    }
    ...
    public void setPriority(String sourcename, boolean pri) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_PRIORITY,
                sourcename, pri));
        commandThread.interrupt();
    }
    ...
    public void setLooping(String sourcename, boolean lp) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_LOOPING,
                sourcename, lp));
        commandThread.interrupt();
    }
    ...
    public void setAttenuation(String sourcename, int model) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_ATTENUATION,
                sourcename, model));
        commandThread.interrupt();
    }
    ...
    public void setDistOrRoll(String sourcename, float dr) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_DIST_OR_ROLL,
                sourcename, dr));
        commandThread.interrupt();
    }
    ...
    public void changeDopplerFactor(float dopplerFactor) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.CHANGE_DOPPLER_FACTOR,
                dopplerFactor));
        commandThread.interrupt();
    }
    ...
    public void changeDopplerVelocity(float dopplerVelocity) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.CHANGE_DOPPLER_VELOCITY,
                dopplerVelocity));
        commandThread.interrupt();
    }
    ...
    public void setVelocity(String sourcename, float x, float y, float z) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_VELOCITY,
                sourcename, x, y, z));
        commandThread.interrupt();
    }
    ...
    public void setListenerVelocity(float x, float y, float z) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_LISTENER_VELOCITY,
                x, y, z));
        commandThread.interrupt();
    }
    ...
    public void feedRawAudioData(String sourcename, byte[] buffer) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.FEED_RAW_AUDIO_DATA,
                sourcename, buffer));
        commandThread.interrupt();
    }
    ...
    public void pause(String sourcename) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.PAUSE, sourcename));
        commandThread.interrupt();
    }
    ...
    public void stop(String sourcename) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.STOP, sourcename));
        commandThread.interrupt();
    }
    ...
    public void rewind(String sourcename) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.REWIND, sourcename));
        commandThread.interrupt();
    }
    ...
    public void flush(String sourcename) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.FLUSH, sourcename));
        commandThread.interrupt();
    }
    ...
    public void cull(String sourcename) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.CULL, sourcename));
        commandThread.interrupt();
    }
    ...
    public void activate(String sourcename) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.ACTIVATE, sourcename));
        commandThread.interrupt();
    }
    ...
    public void setTemporary(String sourcename, boolean temporary) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_TEMPORARY,
    ...
    public void removeSource(String sourcename) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.REMOVE_SOURCE,
                sourcename));
        commandThread.interrupt();
    }
    ...
    public void moveListener(float x, float y, float z) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.MOVE_LISTENER,
                x, y, z));
        commandThread.interrupt();
    }
    ...
    public void setListenerPosition(float x, float y, float z) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_LISTENER_POSITION,
                x, y, z));
        commandThread.interrupt();
    }
    ...
    public void turnListener(float angle) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.TURN_LISTENER,
                angle));
        commandThread.interrupt();
    }
    ...
    public void setListenerAngle(float angle) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_LISTENER_ANGLE,
                angle));
        commandThread.interrupt();
    }
    ...
    public void setListenerOrientation(float lookX, float lookY, float lookZ,
        ...
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_LISTENER_ORIENTATION,
    ...
    public void setMasterVolume(float value) {
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.SET_MASTER_VOLUME,
                value));
        commandThread.interrupt();
    }
    ...
    public boolean switchLibrary(Class libraryClass)
        ...
        synchronized (SoundSystemConfig.THREAD_SYNC) {
            ...
            HashMap<String, paulscode.sound.Source> sourceMap = null;
            ListenerData listenerData = null;

            boolean wasMidiChannel = false;
            paulscode.sound.MidiChannel midiChannel = null;
            paulscode.sound.FilenameURL midiFilenameURL = null;
            ...
            try {
                soundLibrary = (paulscode.sound.Library) libraryClass.newInstance();
            ...
            if (errorCheck(soundLibrary == null, "Library null after " +
                ...
                paulscode.sound.SoundSystemException sse = new paulscode.sound.SoundSystemException(
                        className + " did not load properly.  " +
                                "Library was null after initialization.",
                        paulscode.sound.SoundSystemException.LIBRARY_NULL);
    ...
    public boolean newLibrary(Class libraryClass)
        ...
        CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.NEW_LIBRARY,
                libraryClass));
        commandThread.interrupt();
        ...
        if (!initialized(GET, XXX)) {
            ...
            paulscode.sound.SoundSystemException sse = lastException(GET, null);
            if (sse != null)
                throw sse;
        }
        return true;
    ...
    private void CommandNewLibrary(Class libraryClass) {
        ...
        if (errorCheck(soundLibrary == null, "Library null after " +
            ...
            lastException(SET, new paulscode.sound.SoundSystemException(
                    className + " did not load properly.  " +
                            "Library was null after initialization.",
                    paulscode.sound.SoundSystemException.LIBRARY_NULL));
     ...
     */
    private void CommandLoadSound(paulscode.sound.FilenameURL filenameURL) {
        if (soundLibrary != null)
     ...
     * @param identifier What to call the sample.
     */
    private void CommandLoadSound(paulscode.sound.SoundBuffer buffer, String identifier) {
        if (soundLibrary != null)
    ...
    private void CommandQueueSound(String sourcename,
                                   paulscode.sound.FilenameURL filenameURL) {
        if (soundLibrary != null)
     ...
     * @param milis       Number of miliseconds the fadeout should take.
     */
    private void CommandFadeOut(String sourcename, paulscode.sound.FilenameURL filenameURL,
                                long milis) {
        if (soundLibrary != null)
     ...
     * @param milisIn     Number of miliseconds the fadein should take.
     */
    private void CommandFadeOutIn(String sourcename, paulscode.sound.FilenameURL filenameURL,
                                  long milisOut, long milisIn) {
        if (soundLibrary != null)
    ...
    private void CommandNewSource(boolean priority, boolean toStream,
                                  ...
                                  paulscode.sound.FilenameURL filenameURL, float x,
                                  float y, float z, int attModel,
                                  float distORroll) {
        if (soundLibrary != null) {
    ...
    private void CommandQuickPlay(boolean priority, boolean toStream,
                                  boolean toLoop, String sourcename,
                                  paulscode.sound.FilenameURL filenameURL, float x, float y,
                                  float z, int attModel, float distORroll,
                                  boolean temporary) {
        if (soundLibrary != null) {
     ...
     */
    public boolean CommandQueue(paulscode.sound.CommandObject newCommand) {
        synchronized (SoundSystemConfig.THREAD_SYNC) {
            if (newCommand == null) {
                // New command is null - that means execute all queued commands.
                boolean activations = false;
                paulscode.sound.CommandObject commandObject;
                ...
                while (commandQueue != null && commandQueue.size() > 0) {
                    ...
                    if (commandObject != null) {
                        switch (commandObject.Command) {
                            case paulscode.sound.CommandObject.INITIALIZE:
                                CommandInitialize();
                                break;
                            case paulscode.sound.CommandObject.LOAD_SOUND:
                                CommandLoadSound(
                                        (paulscode.sound.FilenameURL) commandObject.objectArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.LOAD_DATA:
                                CommandLoadSound(
                                        (SoundBuffer) commandObject.objectArgs[0],
                                        commandObject.stringArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.UNLOAD_SOUND:
                                CommandUnloadSound(commandObject.stringArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.QUEUE_SOUND:
                                CommandQueueSound(commandObject.stringArgs[0],
                                        (paulscode.sound.FilenameURL) commandObject.objectArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.DEQUEUE_SOUND:
                                CommandDequeueSound(commandObject.stringArgs[0],
                                        commandObject.stringArgs[1]);
                                break;
                            case paulscode.sound.CommandObject.FADE_OUT:
                                CommandFadeOut(commandObject.stringArgs[0],
                                        (paulscode.sound.FilenameURL) commandObject.objectArgs[0],
                                        commandObject.longArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.FADE_OUT_IN:
                                CommandFadeOutIn(commandObject.stringArgs[0],
                                        (paulscode.sound.FilenameURL) commandObject.objectArgs[0],
                                        commandObject.longArgs[0],
                                        commandObject.longArgs[1]);
                                break;
                            case paulscode.sound.CommandObject.CHECK_FADE_VOLUMES:
                                CommandCheckFadeVolumes();
                                break;
                            case paulscode.sound.CommandObject.NEW_SOURCE:
                                ...
                                break;
                            case paulscode.sound.CommandObject.RAW_DATA_STREAM:
                                CommandRawDataStream(
                                        ...
                                        commandObject.floatArgs[3]);
                                break;
                            case paulscode.sound.CommandObject.QUICK_PLAY:
                                ...
                                break;
                            case paulscode.sound.CommandObject.SET_POSITION:
                                CommandSetPosition(commandObject.stringArgs[0],
                                        commandObject.floatArgs[0],
                                        commandObject.floatArgs[1],
                                        commandObject.floatArgs[2]);
                                break;
                            case paulscode.sound.CommandObject.SET_VOLUME:
                                CommandSetVolume(commandObject.stringArgs[0],
                                        commandObject.floatArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.SET_PITCH:
                                CommandSetPitch(commandObject.stringArgs[0],
                                        commandObject.floatArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.SET_PRIORITY:
                                CommandSetPriority(commandObject.stringArgs[0],
                                        commandObject.boolArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.SET_LOOPING:
                                CommandSetLooping(commandObject.stringArgs[0],
                                        commandObject.boolArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.SET_ATTENUATION:
                                CommandSetAttenuation(commandObject.stringArgs[0],
                                        commandObject.intArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.SET_DIST_OR_ROLL:
                                CommandSetDistOrRoll(commandObject.stringArgs[0],
                                        commandObject.floatArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.CHANGE_DOPPLER_FACTOR:
                                CommandChangeDopplerFactor(
                                        commandObject.floatArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.CHANGE_DOPPLER_VELOCITY:
                                CommandChangeDopplerVelocity(
                                        commandObject.floatArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.SET_VELOCITY:
                                ...
                                break;
                            case paulscode.sound.CommandObject.SET_LISTENER_VELOCITY:
                            ...
                            // later:
                            //------------------------------------------------------
                            case paulscode.sound.CommandObject.PLAY:
                                sourcePlayList.add(commandObject);
                                break;
                            case paulscode.sound.CommandObject.FEED_RAW_AUDIO_DATA:
                                sourcePlayList.add(commandObject);
                                break;
                            //------------------------------------------------------
                            case paulscode.sound.CommandObject.PAUSE:
                                CommandPause(commandObject.stringArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.STOP:
                                CommandStop(commandObject.stringArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.REWIND:
                                CommandRewind(commandObject.stringArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.FLUSH:
                                CommandFlush(commandObject.stringArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.CULL:
                                CommandCull(commandObject.stringArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.ACTIVATE:
                                activations = true;
                                CommandActivate(commandObject.stringArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.SET_TEMPORARY:
                                CommandSetTemporary(commandObject.stringArgs[0],
                                        commandObject.boolArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.REMOVE_SOURCE:
                                CommandRemoveSource(commandObject.stringArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.MOVE_LISTENER:
                                CommandMoveListener(commandObject.floatArgs[0],
                                        commandObject.floatArgs[1],
                                        commandObject.floatArgs[2]);
                                break;
                            case paulscode.sound.CommandObject.SET_LISTENER_POSITION:
                                CommandSetListenerPosition(
                                        commandObject.floatArgs[0],
                                        commandObject.floatArgs[1],
                                        commandObject.floatArgs[2]);
                                break;
                            case paulscode.sound.CommandObject.TURN_LISTENER:
                                CommandTurnListener(commandObject.floatArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.SET_LISTENER_ANGLE:
                                CommandSetListenerAngle(
                                        commandObject.floatArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.SET_LISTENER_ORIENTATION:
                                ...
                                break;
                            case paulscode.sound.CommandObject.SET_MASTER_VOLUME:
                                CommandSetMasterVolume(
                                        commandObject.floatArgs[0]);
                                break;
                            case paulscode.sound.CommandObject.NEW_LIBRARY:
    ...
    public boolean playing(String sourcename) {
        synchronized (SoundSystemConfig.THREAD_SYNC) {
            ...
            paulscode.sound.Source src = soundLibrary.getSources().get(sourcename);
    ...
    public boolean playing() {
        synchronized (SoundSystemConfig.THREAD_SYNC) {
            ...
            HashMap<String, paulscode.sound.Source> sourceMap = soundLibrary.getSources();
            if (sourceMap == null)
                return false;
     ...
     */
    private HashMap<String, paulscode.sound.Source> copySources(HashMap<String,
            paulscode.sound.Source> sourceMap) {
        Set<String> keys = sourceMap.keySet();
        Iterator<String> iter = keys.iterator();
        String sourcename;
        paulscode.sound.Source source;
        ...
        HashMap<String, paulscode.sound.Source> returnMap = new HashMap<String, paulscode.sound.Source>();

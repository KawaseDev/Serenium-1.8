...
public class MidiChannel implements MetaEventListener {
     ...
     */
    private paulscode.sound.FilenameURL filenameURL;
    /**
     ...
     */
    private LinkedList<paulscode.sound.FilenameURL> sequenceQueue = null;
    /**
    ...
    public MidiChannel(boolean toLoop, String sourcename, String filename) {
        ...
        filenameURL(SET, new paulscode.sound.FilenameURL(filename));
    ...
    public MidiChannel(boolean toLoop, String sourcename, URL midiFile,
        ...
        // save information about the source:
        filenameURL(SET, new paulscode.sound.FilenameURL(midiFile, identifier));
        sourcename(SET, sourcename);
        setLooping(toLoop);
    ...
    public MidiChannel(boolean toLoop, String sourcename,
                       paulscode.sound.FilenameURL midiFilenameURL) {
        // let others know we are busy loading
        loading(SET, true);
     ...
     */
    public void queueSound(paulscode.sound.FilenameURL filenameURL) {
        if (filenameURL == null) {
        ...
        synchronized (sequenceQueueLock) {
            if (sequenceQueue == null)
                sequenceQueue = new LinkedList<paulscode.sound.FilenameURL>();
    ...
    public void dequeueSound(String filename) {
        ...
        synchronized (sequenceQueueLock) {
            if (sequenceQueue != null) {
                ListIterator<paulscode.sound.FilenameURL> i = sequenceQueue.listIterator();
                while (i.hasNext()) {
     ...
     */
    public void fadeOut(paulscode.sound.FilenameURL filenameURL, long milis) {
        if (milis < 0) {
     ...
     */
    public void fadeOutIn(paulscode.sound.FilenameURL filenameURL, long milisOut,
                          long milisIn) {
        if (filenameURL == null) {
     ...
     */
    public paulscode.sound.FilenameURL getFilenameURL() {
     ...
     */
    public void setFilenameURL(paulscode.sound.FilenameURL value) {
     ...
     */
    private synchronized paulscode.sound.FilenameURL filenameURL(boolean action,
                                                                 paulscode.sound.FilenameURL value) {
        if (action == SET)
            filenameURL = value;
        return filenameURL;
    }

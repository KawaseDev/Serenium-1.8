...
import io.github.nyghtfull.event.events.impl.game.GameLoopEvent;
...
import io.github.nyghtfull.ui.gui.SereniumMainMenu;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    public void run() {
        ...
        while (true) {
            try {
                while (this.running) {
                    Serenium.getInstance().getEventManager().call(new GameLoopEvent());
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        Serenium.getInstance().init();

        if (this.serverName != null) {
            this.displayGuiScreen(new GuiConnecting(new GuiMainMenu(), this, this.serverName, this.serverPort));
        } else {
            this.displayGuiScreen(new SereniumMainMenu());
        }
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn) {
        ...
        if (guiScreenIn == null && this.theWorld == null) {
            guiScreenIn = new SereniumMainMenu();
    ...
    private void runGameLoop() throws IOException {
        ...
        for (int j = 0; j < this.timer.elapsedTicks; ++j) {
            Serenium.getInstance().getEventManager().call(new TickEvent());
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    if (this.currentScreen != null) {
                        ...
                        //Serenium.getInstance().getModuleManager().getObjects().stream().filter(module -> module.getKeyCode() == k).forEach(Module::toggle);
                        Serenium.getInstance().getEventManager().call(new KeyboardEvent(k));
